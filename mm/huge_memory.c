			...
			struct page *page, gfp_t gfp)
{
	...
	if (unlikely(!pmd_none(*vmf->pmd))) {
		...
		if (sysctl_numa_balancing_mode & NUMA_BALANCING_TIERED_MEMORY) {
		    if (page != NULL && page_to_nid(page) == 0)
			count_vm_events(PGALLOC_FAST_TIER, HPAGE_PMD_NR);
		    else
			count_vm_events(PGALLOC_SLOW_TIER, HPAGE_PMD_NR);
		}
